...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const res = await fetch(
    `http://localhost:3000/xrpc/app.netlify.stellarbsky.getReaction?uri=${post.uri}&cid=${post.cid}&limit=50`
  );
  ...
  const repliesWithReactions = await Promise.all(
    replies.map(async (reply) => {
      const res = await fetch(
        `http://localhost:3000/xrpc/app.netlify.stellarbsky.getReaction?uri=${reply.post.uri}&cid=${reply.post.cid}&limit=50`
      );
      const json: { reactions: Reaction[] } = await res.json();
      return {
        ...reply,
